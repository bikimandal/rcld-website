...
function PersonalisedCourses() {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(
        "/api/personalised-courses/supabase.updaterow",
        {
          method: "POST",
          headers: {
  ...
  const handleConfirmDelete = async (id) => {
    ...
    const deletePromise = new Promise(async (resolve, reject) => {
      try {
        const response = await fetch(
          "/api/personalised-courses/supabase.deleterow",
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleNewRow = async () => {
    ...
    try {
      const response = await fetch(
        "/api/personalised-courses/supabase.addingnewrow",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
              ...
              <tbody>
                {courses.map((course, index) => (
                  <tr
                    ...
                    } hover:bg-blue-200 dark:hover:bg-blue-200 hover:text-gray-800 transition-all`}
                  >
...
export default withAuth(PersonalisedCourses);
